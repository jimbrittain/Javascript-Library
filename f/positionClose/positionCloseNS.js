"use strict";
/* global __imns, window, document, jquery, $ */
var adr = __imns('util.tools');
// var adr = window; //for stand-alone delete above and uncomment this line
if(!('positionClose' in adr)){
     ...
     */
    adr.positionClose = function(obj){
        ...
        if('alignments' in obj){
            if(typeof(obj.alignments) === 'object' && (obj.alignments instanceof Array)){ //based on andrewpeace.com is_array
                for(var i=0, imax = obj.alignments.length; i < imax; i += 1){
                    if(obj.alignments[i] instanceof uc.Alignment) { alignments.push(obj.alignments[i]); }}
            } else if(obj.alignments instanceof uc.Alignment){
                alignments.push(obj.alignments);
            } else { alignments.push(new uc.Alignment()); }}
        var elementBounds = new uc.Boundary({
            top: 0,
            left: 0,
            bottom: element.outerHeight(),
            right: element.outerWidth()});
        var documentBounds = new uc.Boundary({
            top: 0,
            left: 0,
        ...
        if(documentBounds.width() < elementBounds.width() || documentBounds.height() < elementBounds.height()){
            ...
            for(var j=0, jmax = alignments.length; j<jmax; j += 1){
                var targetBounds = new uc.Boundary({});
