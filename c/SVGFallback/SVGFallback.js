"use strict";
/* jshint sub:true */
/* global findElementByID, IMDebugger, fetter, setInterval, clearInterval, isHTMLElement, document, window, Image, setTimeout, clearTimeout, navigator */
/* 
...
*/
function SVGFallback(a){
	if(SVGFallback.prototype.singleton){ return SVGFallback.prototype.singleton; }
	SVGFallback.prototype.singleton = this;
...
SVGFallback.prototype.insertBloatedSVGInclusion = function(){
	...
	if(theHead !== null && findElementByID(this.svgScriptId) === undefined){
		var c = this;
		...
		if(findElementByID(this.svgScriptId) !== undefined){
			this.checkLoadingSVGInclusion();
		} else {
			(new IMDebugger()).pass("SVGFallback: Document does not have or allow access to the DOM/Head tag.");
		}}};

SVGFallback.prototype.loadedSVGInclusion = function(){
	this.ready = true;
	this.objectSupported = true;
	this.processSVGImages();
	if(this.svgScriptInterval !== null){ clearInterval(this.svgScriptInterval); }};

SVGFallback.prototype.checkLoadingSVGInclusion = function(){ 
	var elem = findElementByID(this.svgScriptId);
	if(elem !== undefined && isHTMLElement(elem) && elem.readyState === 'complete'){ this.loadedSVGInclusion(); }};

SVGFallback.prototype.isSVGObjectSupported = function(){
	if(document.all && (parseFloat(navigator.appVersion.split('MSIE ')[1]) < 9) && !('opera' in window)){
		this.processSVGObjectDowngrade();
		return false;
	} else { return true; }};

SVGFallback.prototype.processSVGObjectDowngrade = function(){
	var objArray = [];
...
SVGFallback.prototype.isSVGObject = function(el){
	if(isHTMLElement(el)){
		...
		(new IMDebugger()).pass("SVGFallback.isSVGObject: Needs to be supplied a valid HTML Element");
	}};

SVGFallback.prototype.hasElementBeenProcessed = function(el){
	if(isHTMLElement(el)){
		...
		(new IMDebugger()).pass("SVGFallback.hasElementBeenProcessed: Needs to be supplied a valid HTML Element");
	}};

SVGFallback.prototype.downGradeSVGObject = function(el){
...
SVGFallback.prototype.isSVGImage = function(el){
	var theTag = ('tagName' in el) ? el.tagName : " ";
...
SVGFallback.prototype.addEventHandler = function(obj, fun, evt){
...
SVGFallback.prototype.svgImageError = function(){
	this.imageTested = true;
	this.imageSupported = false;
	this.insertBloatedSVGInclusion(); };

SVGFallback.prototype.svgImageLoad = function(el){
...
SVGFallback.prototype.svgImageReadyState = function(el){
	if('readyState' in el){
...
SVGFallback.prototype.isSVGImageSupported = function(el){
	if(this.imageTested){
		return this.imageSupported;
	} else {
		var isComplete = (el !== undefined && isHTMLElement(el) && this.isSVGImage(el) && 'complete' in el)? el.complete : '';
		if(isComplete === ''){
...
SVGFallback.prototype.processSVGImages = function(){
	var imgArray = [];
	if('getElementsByTagName' in document){
...
SVGFallback.prototype.testAndSetAttribute = function(el, attr, val){
	var tested = false;
	var exists = false;
	if(!document.all){
...
SVGFallback.prototype.replaceSVGImage = function(el){
	if(isHTMLElement(el) && this.isSVGImage(el)){
...
SVGFallback.prototype.init = function(){
	var c = this;
	if(arguments && arguments.length === 1){
	...
	fetter(window, 'load', fireOn, true); };
