...
function EmailValidator(){
	if(EmailValidator.prototype.singleton !== undefined){ return EmailValidator.prototype.singleton; }
	EmailValidator.prototype.singleton = this; }
EmailValidator.prototype.localStringCheck = function(str){
	var good = false, re=/ /;
...
EmailValidator.prototype.ipAddressFormatCheck = function(str){ //check if domian is IP Address
	...
	if(addressRecords.length !== 4){
		...
		for(i=0, imax = addressRecords.length; i < imax; i+=1){
			...
			if(isNumber(onar) && Math.round(onar) !== onar){
				if(onar > -1 && onar < 256){ //check that numbers are in the correct range;
					continue;
				} else { return false; }
			} else { return false; }}}
	return true; };
EmailValidator.prototype.checkEmailLocalPart = function(str){
			...
			if(str.indexOf('"') === -1){  //checks if any quotes
				...
				if(quoteNumber.length === 3){ //checks if exactly two quotes
					if(str.charAt(0) === '"' && str.charAt(str.length - 1) === '"'){ // If string length <=64 check if exactly 2 quotes & starts with quotes
						return true; }}}}}
	return false; };
EmailValidator.prototype.checkEmailDomainPart = function(str){
	...
	return false; };
EmailValidator.prototype.ipNewRegExp = function(str){
	if(typeof str === 'string'){
		var re = /^\[((\d|[a-fA-F]){0,4}\:)+((\d|[a-fA-F]){0,4}\])$/;
		return (re.test(str) === true) ? true : false; // Non-allowed char found or section too long
	} else { return false; }};
EmailValidator.prototype.domainRegExp = function(str){
	if(typeof str === 'string'){
		var re = /^\w+([\.-]?\w+)*\.+(\w{2,63})+$/i;
		return (re.test(str) === true) ? true : false;
	} else { return false; }};
EmailValidator.prototype.validate = function(theEmail){
	var str = theEmail;
	if(typeof str === 'string'){
...
function validatePassedEmail(theEmail){ return (new EmailValidator()).validate(theEmail); }